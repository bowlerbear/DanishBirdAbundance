...
box = c(xmin = 441745.6, ymin = 6049775.1, xmax = 852801.1, ymax = 6402206.9)
plot(st_crop(Denmark, st_bbox(box)))
Denmark <- st_crop(Denmark, st_bbox(box))
...
res(r)
#r <- aggregate(r, fact=5,fun="modal")
r[r==999999] <- NA
#sort(unique(values(r)))#corresponds with table 3.6!!
#table(values(r))
plot(r)
...
getLanduse <- function(start=1, end=100){
  
mypolysS <- mypolys[200001:212702,]
rS <- crop(raster::raster(r),extent(mypolysS))
#plot(rS)
#plot(mypolysS,add=T)

squares_buffer_landUse <- raster::extract(rS,
                                          mypolysS,
                                          df=T,
                                          weights=TRUE)
nrow(squares_buffer_landUse)
names(squares_buffer_landUse)[2] <- "LandUse"
squares_buffer_landUse <- subset(squares_buffer_landUse, !is.na(LandUse))
...
return(squares_buffer_landUse_summary)

}

#read in each set and see how long it is
squares_buffer_landUse_summary <- list.files() %>%
                                  str_subset("set") %>%
                                  str_subset("dataset", negate=TRUE) %>%
                                  map_dfr(readRDS)
...
saveRDS(squares_buffer,file="environ-data/grid_environdata_uneven.rds")
