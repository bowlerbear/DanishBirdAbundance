...
# read in color data
color <- read_csv("dale_color_data/Dale_2015_data_allpoints.csv")

# how many species
length(unique(color$TipLabel))

# read in a list of species in the traits
species_list <- readRDS("avonet_traits_birdtree_taxonomy.RDS") %>%
  dplyr::select(TipLabel)

# filter the color to only the species we have in the danish birds dataset
color <- color %>%
  dplyr::filter(TipLabel %in% species_list$TipLabel)
...
color.complete %>%
  ...
  mutate(distance_from_brown = sqrt(((L - brown_L) ^ 2) + ((a - brown_a) ^ 2) +
...
############ distance from brown doesn't seem to be working super well
...
color_summary.2 <- color_summary %>%
  ...
  mutate(combined_color=max_distance+max_brightness) %>%
  ungroup() %>%
  mutate(combined_color=scales::rescale(combined_color))
...
write_csv(color_summary.2,
          "Data/dale_color_data/color_data_by_species.csv")
